...
# 목표 : 우리의 gene expression matrix에 새로운 column으로 probe ID에 대응하는 gene symbol을 넣고 싶음_ Retrive mapping from probe ID to symbols
# 이를 가능하게 하는 mapper는 [HG-U133A_2]로 이 패키지의 설치가 필요
BiocManager::install("hgu133a2.db")
library(hgu133a2.db)
# x는 특정한 key(probe ID)를 받아서 그에 맞는 gene symbol을 return해주는 mapper
x <- hgu133a2SYMBOL
...
# 문제 2 : expression은 현재 matrix (모든 요소가 같은 type) 이기 때문에 character vector인 symbols를 새로운 column ⇒ 전체 데이터가 characters
